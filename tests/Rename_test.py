...
class Renamer_test(unittest.TestCase):
    def test_renamer(self):
        with open(os.path.join(os.path.dirname(__file__), 'files/input.json')) as input_file:
            torrents_name = json.load(input_file)
        with open(os.path.join(os.path.dirname(__file__), 'files/output.json')) as output_file:
